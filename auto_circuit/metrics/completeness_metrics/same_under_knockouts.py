...
from auto_circuit.types import AlgoKey, Edge, PruneScores, TaskKey, TaskPruneScores
import torch as t
from torch.nn.functional import log_softmax, kl_div
from auto_circuit.utils.custom_tqdm import tqdm
import plotly.express as px
from auto_circuit.utils.graph_utils import get_sorted_src_outs, mask_fn_mode, patch_mode, set_all_masks, train_mask_mode
...
def same_under_knockouts_fig(
    ...
    fig = subplots.make_subplots(
        rows=1,
        cols=n_cols,
        subplot_titles=[TASK_DICT[task].name for task in task_completeness_scores.keys()],
    )
    for col, (task_key, algo_completeness_scores) in enumerate(task_completeness_scores.items(), start=1):
        for algo_key, completeness_scores in algo_completeness_scores.items():
            fig.add_trace(
                go.Scatter(
                    x=[n_mask for n_mask, _ in completeness_scores] + [(x + y) / 2 for x, y in completeness_scores],
                    y=[avg_kl_div for _, avg_kl_div in completeness_scores] + [(x + y) / 2 for x, y in completeness_scores],
        ...
        min_x = min([min([n_mask for n_mask, _ in completeness_scores]) for completeness_scores in algo_completeness_scores.values()])
        max_x = max([max([(x + y) / 2 for x, y in completeness_scores]) for completeness_scores in algo_completeness_scores.values()])
...
def run_same_under_knockout(
    ...
    corrupt_src_outs_dict: Dict[int, t.Tensor] = {}
    ...
    with train_mask_mode(model) as patch_masks:
        ...
        for epoch in (epoch_pbar := tqdm(range(epochs))):
            epoch_pbar.set_description_str(f"Epoch: {epoch}, KL Div: {kl_div_history[-1] if len(kl_div_history) > 0 else None}")
            for batch in task.test_loader:
                patch_outs = corrupt_src_outs_dict[batch.key].clone().detach()
                with patch_mode(model, t.zeros_like(patch_outs), patch_outs), mask_fn_mode(model, mask_fn="hard_concrete"):
                    optim.zero_grad()
                    model.zero_grad()
                    with t.no_grad():
                        for circuit_mask, patch_mask in zip(circuit_masks, patch_masks):
                            t.where(circuit_mask, patch_mask, t.ones_like(patch_mask) * 99, out=patch_mask)
                    circuit_logprobs = log_softmax(model(batch.clean)[model.out_slice], dim=-1)
                    with t.no_grad():
                        for circuit_mask, patch_mask in zip(circuit_masks, patch_masks):
                            t.where(circuit_mask, patch_mask, t.ones_like(patch_mask) * -99, out=patch_mask)
                    model_logprobs = log_softmax(model(batch.clean)[model.out_slice], dim=-1)
                    kl_div_term = -kl_div(circuit_logprobs, model_logprobs, reduction="batchmean", log_target=True)
                    knockouts_samples = sample_hard_concrete(t.cat([patch_mask.flatten() for patch_mask in patch_masks]), batch_size=1)
                    reg_term = (t.relu(knockouts_samples.sum() - n_knockouts) / n_knockouts) * regularize_lambda
        ...
        fig.update_layout(title=f"Same Under Knockouts for Task: {task.name}, Algo: {algo.name}")
        fig.show()

        knockout_circuit_logprobs, knockout_model_logprobs, circuit_logprobs, model_logprobs = [], [], [], []
        with t.inference_mode(), mask_fn_mode(model, mask_fn=None):
            # Discretize the circuit with knockouts
            for circuit_mask, patch_mask in zip(circuit_masks, patch_masks):
                t.where(patch_mask >= hard_concrete_threshold, t.ones_like(patch_mask), t.zeros_like(patch_mask), out=patch_mask)
                t.where(circuit_mask, patch_mask, t.ones_like(patch_mask), out=patch_mask)
            # Test the circuit with the knockouts
            for batch in task.test_loader:
                patch_outs = corrupt_src_outs_dict[batch.key].clone().detach()
                with patch_mode(model, t.zeros_like(patch_outs), patch_outs), t.inference_mode():
                    knockout_circuit_logprobs.append(log_softmax(model(batch.clean)[model.out_slice], dim=-1))

            # Test the full model with the same knockouts
            for circuit_mask, patch_mask in zip(circuit_masks, patch_masks):
                t.where(circuit_mask, patch_mask, t.zeros_like(patch_mask), out=patch_mask)
            knockouts_size = int(sum([mask.sum().item() for mask in patch_masks]))
            for batch in task.test_loader:
                patch_outs = corrupt_src_outs_dict[batch.key].clone().detach()
                with patch_mode(model, t.zeros_like(patch_outs), patch_outs), t.inference_mode():
                    knockout_model_logprobs.append(log_softmax(model(batch.clean)[model.out_slice], dim=-1))
                
            # Test the circuit without knockouts (with tree patching)
            for circuit_mask, patch_mask in zip(circuit_masks, patch_masks):
                t.where(circuit_mask, t.zeros_like(patch_mask), t.ones_like(patch_mask), out=patch_mask)
            for batch in task.test_loader:
                patch_outs = corrupt_src_outs_dict[batch.key].clone().detach()
                with patch_mode(model, t.zeros_like(patch_outs), patch_outs), t.inference_mode():
                    circuit_logprobs.append(log_softmax(model(batch.clean)[model.out_slice], dim=-1))
            
            # Test the full model without knockouts
            for batch in task.test_loader:
                model_logprobs.append(log_softmax(model(batch.clean)[model.out_slice], dim=-1))

    avg_knockout_kl_div = kl_div(t.cat(knockout_circuit_logprobs), t.cat(knockout_model_logprobs), reduction="batchmean", log_target=True).item()
    avg_normal_kl_div = kl_div(t.cat(circuit_logprobs), t.cat(model_logprobs), reduction="batchmean", log_target=True).item()
    print("model n_edges", len(model.edges), "knockouts_size", knockouts_size, "circuit_size", circuit_size, "diff", circuit_size - knockouts_size)
    print("avg_knockout_kl_div", avg_knockout_kl_div, "avg_normal_kl_div", avg_normal_kl_div)
