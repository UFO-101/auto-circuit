...
from auto_circuit.types import ActType, Edge, EdgeSrc, ExperimentType
from auto_circuit.utils.graph_utils import graph_edges, edge_counts_util
...
def acdc_prune_scores(
    ...
    for tao in (pbar_tao:=tqdm(tao_values)):
        ...
        for edge in (pbar_edge:=tqdm(edges)):
            ...
            pruned_outs = run_pruned(model, factorized, train_data, exp_type, [edge_count], edges_to_prune)
...
def acdc_edge_counts(model: t.nn.Module, factorized: bool, prune_scores: Dict[Edge, float]) -> List[int]:
    ...
    prune_scores_count = dict(sorted(prune_scores_count.items(), key=lambda item: item[0]))
    # Create edge_counts
    edge_counts = []
