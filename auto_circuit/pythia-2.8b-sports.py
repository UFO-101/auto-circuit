...
min_name_count = min(len(football_valid_players), len(basketball_valid_players), len(baseball_valid_players))
...
for ans_idx, (sport, sport_resids) in enumerate(resids.items()):
    ...
    fig.update_layout(title="Average Probability of Each Sport", xaxis_title="Layer", yaxis_title="Probability")
    fig.show()

#%%

layer_2_resids = t.cat([r[2].detach().clone() for r in resids.values()])
correct_dirs = t.cat([probe[:, ans_idx].detach().clone().repeat(len(r[2]), 1) for ans_idx, r in enumerate(resids.values())])
answer_idxs = t.cat([t.tensor([ans_idx] * len(r[2])) for ans_idx, r in enumerate(resids.values())])
