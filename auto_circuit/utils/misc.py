...
def set_module_by_name(model: t.nn.Module, module_name: str, new_module: t.nn.Module) -> None:
    parent = model
    if "." in module_name:
...
def percent_gpu_mem_used(total_gpu_mib: int = 49000) -> str:
    return "Memory used {:.1f}".format(((t.cuda.memory_allocated() / (2**20)) / total_gpu_mib) * 100) + "%"