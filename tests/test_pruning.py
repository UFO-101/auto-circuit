...
def test_pruning(setup_data: Dict[str, Any]):
    ...
    assert t.allclose(clean_out, t.tensor([16.0]))
    assert t.allclose(corrupt_out, t.tensor([-16.0]))
    ...
    assert t.allclose(pruned_outs[1][0], t.tensor([12.0]), atol=1e-3)
    assert t.allclose(pruned_outs[2][0], t.tensor([8.0]), atol=1e-3)
    assert t.allclose(pruned_outs[3][0], t.tensor([4.0]), atol=1e-3)
